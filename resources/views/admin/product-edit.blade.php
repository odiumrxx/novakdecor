        ...
        <div class="main-content-wrap">
            <div class="flex items-center flex-wrap justify-between gap20 mb-27">
                ...
                <ul class="breadcrumbs flex items-center flex-wrap justify-start gap10">
                    ...
                    <li>
                        <div class="text-tiny">Изменить продукт</div>
                    </li>
                </ul>
            </div>
            <form class="tf-section-2 form-add-product" method="POST" enctype="multipart/form-data"
                ...
                <div class="wg-box">
                    ...
                    <fieldset class="description">
                        ...
                        <textarea class="mb-10" name="description" placeholder="Описание" tabindex="0" aria-required="true" required>{{ old('description', $product->description) }}</textarea>
                ...
                <div class="wg-box">
                    ...
                    <fieldset>
                        ...
                        <div class="upload-image mb-16">
                            <div class="item" id="imgpreview" style="{{ $product->image ? '' : 'display:none' }}">
                                ...
                                <img src="{{ $product->image ? asset('uploads/products/' . $product->image) : asset('images/upload/upload-1.png') }}"
                                    class="effect8" alt="Изображение продукта">
                                @if ($product->image)
                                    <button type="button" class="remove-image-btn" data-type="main-image"><i
                    ...
                    <fieldset>
                        ...
                        <div class="upload-image mb-16 flex gap10 mb-10" id="gallery-preview-container">
                            {{-- Существующие изображения галереи (если есть) --}}
                            @if ($product->images)
                            ...
                            <div class="select mb-10">
                                <select class="" name="stock_status">
                                    <option value="instock"
                                        {{ old('stock_status', $product->stock_status) == 'instock' ? 'selected' : '' }}>В
                                        наличии</option>
                        ...
                        <fieldset class="name">
                            ...
                            <div class="select mb-10">
                                <select class="" name="featured">
                                    <option value="0"
                                        {{ old('featured', $product->featured) == '0' ? 'selected' : '' }}>Нет</option>

                                    <option value="1"
                                        {{ old('featured', $product->featured) == '1' ? 'selected' : '' }}>Да</option>
    ...
    <script>
        $(function() {
            ...
            $("#gFile").on("change", function(e) {
                ...
                // Получаем контейнер для предпросмотра изображений галереи.
                // Теперь это родительский div upload-image с новым ID 'gallery-preview-container'.
                const galleryPreviewContainer = $("#gallery-preview-container");

                if (files.length > 0) {
                    for (let i = 0; i < files.length; i++) {
                        const file = files[i];
                        // Создаем контейнер для каждого изображения
                        const itemDiv = $('<div>').addClass('item gitems');
                        const img = $('<img>').attr('src', URL.createObjectURL(file)).addClass('effect8');

                        itemDiv.append(img); // Добавляем только изображение
